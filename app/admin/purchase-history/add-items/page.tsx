...
export default function AddMissedPurchaseItemsPage() {
  const searchParams = useSearchParams();
  const purchaseId = searchParams.get("purchaseId") || "";

  const [targetPurchase, setTargetPurchase] =
    useState<ExistingPurchaseTarget | null>(null);
  ...
  useEffect(() => {
    ...
    async function loadTargetPurchase() {
      if (!purchaseId) {
        setTargetError(
          "Purchase ID is missing. Open this page from Purchase History.",
        );
      ...
      try {
        ...
        const purchaseRow =
  data as unknown as ExistingPurchaseTarget;
        setTargetPurchase(purchaseRow);

        setSelectedSupplierId(
          purchaseRow.supplier_id != null
            ? Number(purchaseRow.supplier_id)
            : null,
        );
        setSupplierName(purchaseRow.supplier_name || "");
        setSupplierSearch(purchaseRow.supplier_name || "");
        setSupplierPhone(purchaseRow.supplier_phone || "");
        setSupplierGstin(purchaseRow.supplier_gstin || "");
        setSupplierState(
          purchaseRow.supplier_state || "Andhra Pradesh",
        );
        setSupplierStateCode(
          purchaseRow.supplier_state_code || "37",
        );
        setPlaceOfSupply(
          purchaseRow.place_of_supply ||
            purchaseRow.supplier_state ||
            "Andhra Pradesh",
        );
        setTaxType(
          (purchaseRow.tax_type as TaxType) || "intra_state",
        );
      } catch (error) {
        if (!cancelled) {
          setTargetError(
            error instanceof Error
              ? error.message
              : "Unable to load the selected purchase.",
          );
        }
      } finally {
  ...
  async function completePurchase(
    ...
    if (!targetPurchase || !purchaseId) {
      showNotice(
        "Open this page from Purchase History and select a valid purchase.",
        "error",
      );
      return;
    }
    ...
    try {
      ...
      const { data: refreshedPurchase } = await supabase
        .from("purchases")
        .select(
          "id,purchase_number,supplier_id,supplier_name,supplier_phone,supplier_gstin,supplier_state,supplier_state_code,place_of_supply,tax_type,total_amount,paid_amount,due_amount",
        )
        .eq("id", purchaseId)
        .single();
      ...
      if (refreshedPurchase) {
        setTargetPurchase(
          refreshedPurchase as ExistingPurchaseTarget,
        );
      }

      showNotice(
        `${result.purchase_number || targetPurchase.purchase_number || "Purchase"} updated with all missed items.`,
        "success",
      );
    } catch (error) {
      console.error("Add missed purchase items error:", error);
      showNotice(
        error instanceof Error
          ? error.message
          : "Unable to add missed items to this purchase.",
        "error",
      );
    } finally {
      setSaving(false);
  ...
  return (
    <main className="ncsPurchasePage">
      ...
      <section className="ncsHero">
        <div>
          <span>NEW CITY STYLE • PURCHASE CORRECTION</span>
          <h1>Add Missed Purchase Items</h1>
          <p>
            Add bulk stock, size variants or individual-piece
            barcodes to the same existing purchase in one safe
            transaction.
          </p>
        </div>
      ...
      {targetLoading && (
        <div className="ncsLoadError">
          Loading selected purchase...
        </div>
      )}

      {targetError && (
        <div className="ncsLoadError">{targetError}</div>
      )}

      {targetPurchase && (
        <section className="ncsTargetPurchaseStrip">
          <div>
            <span>EXISTING PURCHASE</span>
            <strong>
              {targetPurchase.purchase_number || targetPurchase.id}
            </strong>
          </div>
          <div>
            <span>SUPPLIER</span>
            <strong>{targetPurchase.supplier_name || "—"}</strong>
          </div>
          <div>
            <span>CURRENT TOTAL</span>
            <strong>
              {formatCurrency(
                toNumber(targetPurchase.total_amount),
              )}
            </strong>
          </div>
          <div>
            ...
            <strong>
              {formatCurrency(
                toNumber(targetPurchase.due_amount),
              )}
            </strong>
          </div>
        </section>
      ...
      >
        <section className="ncsPurchaseMain">
          <article className="ncsCard ncsHiddenSupplierStep">
          ...
          <article className="ncsCard">
            <header className="ncsCardHeader">
              <div>
                <span>STEP 2</span>
                <h2>Add All Missed Items</h2>
                <p>
                  Existing barcode/product search చేయండి లేదా
                  missed item rowsలో పూర్తి details enter చేయండి.
                </p>
              </div>
            ...
            >
              ...
              <div>
                <strong>
                  {saving
                    ? "Adding All Items..."
                    : "Add All Items to Same Purchase"}
                </strong>
      ...
      {successPurchase && (
        <div className="ncsModalOverlay">
          <section className="ncsSuccessModal">
            ...
            <span>PURCHASE UPDATED</span>
            <h2>{successPurchase.purchase_number}</h2>
            <p>
              All missed items were added under the same purchase.
              Stock, tax totals and supplier outstanding were updated.
            </p>
      ...
      <style jsx global>{`
        * {
        ...
        .ncsHiddenSupplierStep {
          display: none !important;
        }

        .ncsTargetPurchaseStrip {
          display: grid;
          ...
          border-radius: 16px;
          background: #ffffff;
          box-shadow: 0 12px 30px rgba(3, 21, 63, 0.08);
        }

        .ncsTargetPurchaseStrip > div {
          padding: 11px 12px;
          border-radius: 11px;
          background: #f6f8fc;
        }

        .ncsTargetPurchaseStrip span,
        .ncsTargetPurchaseStrip strong {
          display: block;
        }
